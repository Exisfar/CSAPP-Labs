...
def main():
    ...
    p = subprocess.Popen(
        "./test-trans -M 32 -N 32 | grep TEST_TRANS_RESULTS",
        shell=True,
    ...
    p = subprocess.Popen(
        "./test-trans -M 64 -N 64 | grep TEST_TRANS_RESULTS",
        shell=True,
        stdout=subprocess.PIPE,
    )
    stdout_data = p.communicate()[0].decode("utf-8")  # Decode bytes to string
    ...
    p = subprocess.Popen(
        "./test-trans -M 61 -N 67 | grep TEST_TRANS_RESULTS",
        shell=True,
        stdout=subprocess.PIPE,
    )
    stdout_data = p.communicate()[0].decode("utf-8")  # Decode bytes to string
    ...
    trans32_score = computeMissScore(miss32, 300, 600, maxscore["trans32"]) * int(
        result32[0]
    )
    trans64_score = computeMissScore(miss64, 1300, 2000, maxscore["trans64"]) * int(
        result64[0]
    )
    trans61_score = computeMissScore(miss61, 2000, 3000, maxscore["trans61"]) * int(
        result61[0]
    )
    ...
    print(
        "%-22s%8.1f%10d%12s"
        % ("Trans perf 32x32", trans32_score, maxscore["trans32"], misses)
    )
    ...
    print(
        "%-22s%8.1f%10d%12s"
        % ("Trans perf 64x64", trans64_score, maxscore["trans64"], misses)
    )
    ...
    print(
        "%-22s%8.1f%10d%12s"
        % ("Trans perf 61x67", trans61_score, maxscore["trans61"], misses)
    )
